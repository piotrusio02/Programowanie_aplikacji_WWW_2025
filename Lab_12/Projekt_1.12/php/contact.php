<!-- 
Moduł obsługujący formularz kontaktowy.
Obsługuje logikę formularza kontaktowego oraz funkcję przypominania hasła.
...
<?php
    ...
    // Wymagane pliki do obsługi poczty SMTP.
    ...
    /**
    * PokazKontakt()
    * Wyświetla formularz kontaktowy HTML oraz komunikaty o statusie wysyłki
    */
    ...
    * Obsługuje dane z formularza kontaktowego i wysyła e-mail przez SMTP.
    */
    function WyslijMailKontakt(string $odbiorca) {
        ...
        if (empty($_POST['temat']) || empty($_POST['tresc']) || empty($_POST['email'])) {
            $_SESSION['mail_status_message'] = '<p style="color: red;">Wypełnij wszystkie pola formularza.</p>';
        } else {
		
            $mail = new PHPMailer(true);

            // htmlspecialchars() - Zabezpiecza przed wstrzyknięciem kodu.
            $adres_uzytkownika = htmlspecialchars($_POST['email']);
            $temat_wiadomosci  = htmlspecialchars($_POST['temat']);
            $tresc_wiadomosci  = htmlspecialchars($_POST['tresc']);
            ...
            try {
                ...
                $mail->isHTML(false);
                $mail->Subject = '[PENNE KONTAKT] ' . $temat_wiadomosci . ' (Od: ' . $adres_uzytkownika . ')';
                ...
                $_SESSION['mail_status_message'] = '<p style="color: green;">Twoja wiadomość została wysłana pomyślnie.</p>';
			
            } catch (Exception $e) {
                $_SESSION['mail_status_message'] = '<p style="color: red;">Błąd: ' . $mail->ErrorInfo . '</p>';
            }
        }
    ...
    /**
    * PrzypomnijHaslo($odbiorca, $haslo)
    * Wysyła hasło administratora na zweryfikowany adres e-mail.
    */
    function PrzypomnijHaslo(string $odbiorca, string $haslo) {
        ...
        try {
            ...
            $_SESSION['password_status_message'] = "<p style='color: red;'>Wystąpił błąd: {$mail->ErrorInfo}</p>";
        }
    ...
    // Główna obsługa logiki formularza kontaktowego
